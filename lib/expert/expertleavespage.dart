...
class ExpertLeavesPage extends StatefulWidget {
  @override
  _ExpertLeavesPageState createState() => _ExpertLeavesPageState();
}

List<dynamic> imagesList = [];

class _ExpertLeavesPageState extends State<ExpertLeavesPage> {
  void fetchAllPictures() async {
    try {
      final response = await http.get(
        Uri.parse(Path.globalpath + '/all-pictures'),
      );
                  ...
                  return GestureDetector(
                    onTap: () {
                      Navigator.push(
                        context,
                        MaterialPageRoute(
                          builder: (context) => LeafCommentPage1(
...
class LeafCommentPage1 extends StatefulWidget {
...
class _LeafCommentPageState extends State<LeafCommentPage1> {
  final Uint8List imageBytes;
  final String aiPredictedState;
  final String correspondingTree;
  final int index;
  ...
  void add_comment(
    ...
    var response = await http.post(
      ...
      body: jsonEncode({
        ...
        'ask_expert': imagesList[index]['ask_expert'],
      }),
    );
        ...
        builder: (context) {
          return CustomAlertDialog(
            ...
            onOkPressed: () {
              Navigator.push(
                context,
                MaterialPageRoute(builder: (context) => ExpertPage(2)),
              );
            },
          );
        },
